...
import { burnedFoodInNasiGoreng } from "@drizzle/schema";
...
export const GET = GETMethodRoute(schema, async (_, query) => {
  return (
    await DB_SQL.query.burnedFoodInNasiGoreng.findMany({
      extras: {
        image: sql<string>`${process.env.IMAGE_URL} || ${burnedFoodInNasiGoreng.image}`.as("image")
      },
