...
import { upgradeInNasiGoreng } from "@drizzle/schema";
...
export const GET = GETMethodRoute(schema, async (_, query) => {
  return okResponse(
    await DB_SQL.query.upgradeInNasiGoreng.findMany({
      extras: {
        image: sql<string>`${process.env.IMAGE_URL} || ${upgradeInNasiGoreng.image}`.as("image")
      },
