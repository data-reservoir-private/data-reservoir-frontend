...
import { plateInNasiGoreng } from "@drizzle/schema";
...
export const GET = GETMethodRoute(schema, async (_, query) => {
  return okResponse(
    await DB_SQL.query.plateInNasiGoreng.findMany({
      extras: {
        image: sql<string>`${process.env.IMAGE_URL} || ${plateInNasiGoreng.image}`.as("image")
      },
