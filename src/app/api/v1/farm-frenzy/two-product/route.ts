...
import { twoProductInFarmFrenzy } from "@drizzle/schema";
...
export const GET = GETMethodRoute(schema, async (_, query) => {
  return (
    await DB_SQL.query.twoProductInFarmFrenzy.findMany({
      extras: {
        image: sql<string>`${process.env.IMAGE_URL} || ${twoProductInFarmFrenzy.image}`.as("image")
      },
      limit: query.pageSize === 0 ? undefined : query.pageSize,
      offset: query.pageSize === 0 ? 0 : query.currentPage * query.pageSize
    })
  ) satisfies FarmFrenzyTwoProductResponse[];
}, { cache: true });