...
export type NasiGorengBurnedFoodFormSchema = z.infer<typeof schema>;

export default function NasiGorengBurnedFoodForm({ param, totalData = 0 }: { param: NasiGorengBurnedFoodFormSchema, totalData: number }) {
  const defaultValues = formOptions({
    defaultValues: {
      name: param.name ?? "",
      currentPage: param.currentPage ?? 1,
      pageSize: param.pageSize ?? 50
    } as NasiGorengBurnedFoodFormSchema,
    validators: {
      onChange: schema
    },
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value, meta }) => {
      ...
      router.push(`/nasi-goreng/burned-food?${makeSearchParam(value)}`);
    }
  });
