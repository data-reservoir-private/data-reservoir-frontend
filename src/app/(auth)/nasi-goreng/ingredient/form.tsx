...
export type NasiGorengIngredientFormSchema = z.infer<typeof schema>;

export default function NasiGorengIngredientForm({ param, totalData = 0 }: { param: NasiGorengIngredientFormSchema, totalData: number }) {
  const defaultValues = formOptions({
    ...
    } as NasiGorengIngredientFormSchema,
    validators: {
      onChange: schema
    },
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value, meta }) => {
      ...
      router.push(`/nasi-goreng/ingredient?${makeSearchParam(value)}`);
    }
  });
