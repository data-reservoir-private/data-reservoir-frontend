...
export type NasiGorengUpgradeFormSchema = z.infer<typeof schema>;

export default function NasiGorengUpgradeForm({ param, totalData = 0 }: { param: NasiGorengUpgradeFormSchema, totalData: number }) {
  const defaultValues = formOptions({
    defaultValues: {
      name: param.name ?? "",
      currentPage: param.currentPage ?? 1,
      pageSize: param.pageSize ?? 50
    } as NasiGorengUpgradeFormSchema,
    validators: {
      onChange: schema
    },
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value, meta }) => {
      ...
      router.push(`/nasi-goreng/upgrade?${makeSearchParam(value)}`);
    }
  });
