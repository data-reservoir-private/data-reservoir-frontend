...
export type NasiGorengFriedRiceFormSchema = z.infer<typeof schema>;

export default function NasiGorengFriedRiceForm({ param, totalData = 0 }: { param: NasiGorengFriedRiceFormSchema, totalData: number }) {
  const defaultValues = formOptions({
    ...
    } as NasiGorengFriedRiceFormSchema,
    validators: {
      onChange: schema
    },
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value, meta }) => {
      ...
      router.push(`/nasi-goreng/fried-rice?${makeSearchParam(value)}`);
    }
  });
