...
const grabDetail = cache(
  async (id: string) =>
    await grabData<INasiGorengResponse["fried-rice-complete"] | null>(
      API_ROUTE.NASI_GORENG.FRIED_RICE.ID(id),
    ),
);
...
export default async function NasiGorengFriedRiceDetail(
  props: NasiGorengFriedRiceDetailProps,
) {
  ...
  return (
    <Section
      ...
      breadcrumbs={[
        ...BREADCRUMBS["nasi-goreng-fried-rice-detail"],
        { label: data.name },
      ]}
    >
      {/* Image */}
...
function Levels({
  ...
  level: INasiGorengResponse["fried-rice-complete"]["level"];
}) {
  return (
      ...
      {level.map((l) => (
        <Paper className="flex p-1 max-md:flex-col" key={l.id}>
          <Box className="flex flex-col gap-5 max-md:justify-center max-md:items-center">
            ...
            <Typography className="w-full text-lg text-center">
              Level {l.level}
            </Typography>
          </Box>
                  ...
                  {l.upgrades.map((up) => (
                    <Grid size={1} key={up.id}>
                      <Paper className="flex overflow-hidden">
                        <Link
                          passHref
                          href={`/nasi-goreng/upgrade/${up.id}`}
                          prefetch={false}
                        >
                  ...
                  {l.recipe.map((ing) => (
                    <Grid size={1} key={ing.id}>
                      <Paper className="flex overflow-hidden">
                        <Link
                          passHref
                          href={`/nasi-goreng/ingredient/${ing.id}`}
                          prefetch={false}
                        >
                          <Box className="w-10 h-full min-h-10 relative bg-gray-500/20 hover:bg-gray-600/20 hover:transition-colors">
                            <SimpleImage
                        ...
                        <Box className="grow flex items-center justify-between">
                          <Box className="grow flex">
                            <Box className="grow p-3">
                              <Typography className="">{ing.name}</Typography>
                            </Box>
                          </Box>
                          <Box className="mr-3 px-2.5 py-1 rounded-full bg-gray-700/50 border border-gray-600/50 flex items-center">
                            <Typography
                              variant="body2"
