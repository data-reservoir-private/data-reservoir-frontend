...
export type NasiGorengToolFormSchema = z.infer<typeof schema>;

export default function NasiGorengToolForm({ param, totalData = 0 }: { param: NasiGorengToolFormSchema, totalData: number }) {
  const defaultValues = formOptions({
    defaultValues: {
      name: param.name ?? "",
      currentPage: param.currentPage ?? 1,
      pageSize: param.pageSize ?? 50
    } as NasiGorengToolFormSchema,
    validators: {
      onChange: schema
    },
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value, meta }) => {
      ...
      router.push(`/nasi-goreng/tool?${makeSearchParam(value)}`);
    }
  });
