...
export type NasiGorengRelicFormSchema = z.infer<typeof schema>;

export default function NasiGorengRelicForm({ param, totalData = 0 }: { param: NasiGorengRelicFormSchema, totalData: number }) {
  const defaultValues = formOptions({
    defaultValues: {
      name: param.name ?? "",
      currentPage: param.currentPage ?? 1,
      pageSize: param.pageSize ?? 50
    } as NasiGorengRelicFormSchema,
    validators: {
      onChange: schema
    },
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value, meta }) => {
      ...
      router.push(`/nasi-goreng/relic?${makeSearchParam(value)}`);
    }
  });
