...
export type ArtifactFormSchema = z.infer<typeof schema>;

export default function ArtifactForm({ param, totalData = 0 }: { param: ArtifactFormSchema, totalData: number }) {
  const defaultValues = formOptions({
    defaultValues: {
      name: param.name ?? "",
      currentPage: param.currentPage ?? 1,
      pageSize: param.pageSize ?? 50
    } as ArtifactFormSchema,
    validators: {
      onChange: schema
    },
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value, meta }) => {
      ...
      router.push(`/cygnus/artifact?${makeSearchParam(value)}`);
    }
  });
