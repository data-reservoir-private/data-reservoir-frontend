...
export type MineralFormSchema = z.infer<typeof schema>;

export default function MineralForm({ param, totalData = 0 }: { param: MineralFormSchema, totalData: number }) {
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value, meta }) => {
      ...
      router.push(`/cygnus/mineral?${makeSearchParam(value)}`);
    }
  });
    ...
    >
      <Box className="gap-2 flex grow">
        ...
        <form.AppField name='category' children={(field) => (
          <field.SimpleSelectString label='Category' choices={CATEGORIES['cygnus-mineral']}/>
        )} />
        <form.AppForm>
