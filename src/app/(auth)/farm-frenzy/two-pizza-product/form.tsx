...
export type TwoPizzaProductFormSchema = z.infer<typeof schema>;

export default function TwoPizzaProductForm({ param, totalData = 0 }: { param: TwoPizzaProductFormSchema, totalData: number }) {
  const defaultValues = formOptions({
    defaultValues: {
      name: param.name ?? "",
      currentPage: param.currentPage ?? 1,
      pageSize: param.pageSize ?? 50
    } as TwoPizzaProductFormSchema,
    validators: {
      onChange: schema
    },
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value, meta }) => {
      ...
      router.push(`/farm-frenzy/two-pizza-product?${makeSearchParam(value)}`);
    }
  });
