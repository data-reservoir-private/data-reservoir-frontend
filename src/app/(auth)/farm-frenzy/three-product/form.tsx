...
export type ThreeProductFormSchema = z.infer<typeof schema>;

export default function ThreeProductForm({ param, totalData = 0 }: { param: ThreeProductFormSchema, totalData: number }) {
  const defaultValues = formOptions({
    defaultValues: {
      name: param.name ?? "",
      currentPage: param.currentPage ?? 1,
      pageSize: param.pageSize ?? 50
    } as ThreeProductFormSchema,
    validators: {
      onChange: schema
    },
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value, meta }) => {
      ...
      router.push(`/farm-frenzy/three-product?${makeSearchParam(value)}`);
    }
  });
