...
export type OneProductFormSchema = z.infer<typeof schema>;

export default function OneProductForm({ param, totalData = 0 }: { param: OneProductFormSchema, totalData: number }) {
  const defaultValues = formOptions({
    defaultValues: {
      name: param.name ?? "",
      currentPage: param.currentPage ?? 1,
      pageSize: param.pageSize ?? 50
    } as OneProductFormSchema,
    validators: {
      onChange: schema
    },
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value, meta }) => {
      ...
      router.push(`/farm-frenzy/one-product?${makeSearchParam(value)}`);
    }
  });
