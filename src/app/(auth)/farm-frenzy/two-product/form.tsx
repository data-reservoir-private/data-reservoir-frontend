...
export type TwoProductFormSchema = z.infer<typeof schema>;

export default function TwoProductForm({ param, totalData = 0 }: { param: TwoProductFormSchema, totalData: number }) {
  const defaultValues = formOptions({
    defaultValues: {
      name: param.name ?? "",
      currentPage: param.currentPage ?? 1,
      pageSize: param.pageSize ?? 50
    } as TwoProductFormSchema,
    validators: {
      onChange: schema
    },
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value, meta }) => {
      ...
      router.push(`/farm-frenzy/two-product?${makeSearchParam(value)}`);
    }
  });
