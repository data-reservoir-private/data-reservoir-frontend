...
interface ThreePCSpreadDishDetailProps {
...
export async function generateMetadata(props: ThreePCSpreadDishDetailProps) {
  ...
  return {
    title: `The Sims Three PC Spread Dish - ${post.data.name} - Data Reservoir`
  }
}

export default async function ThreePCSpreadDishDetail(props: ThreePCSpreadDishDetailProps) {
  const { id } = await props.params;
  const { data } = await grabDetail(id);
  if (!data) return notFound();

    return (
    <Section name={data.name} variant='h4' className='flex flex-col gap-3' breadcrumbs={[...BREADCRUMBS['the-sims-three-pc-spread-dish-detail'], { label: data.name }]}>
      {/* Image */}
