...
export default async function FourPCElementDetail(props: FourPCElementDetailProps) {
  ...
  return (
    <Section name={data.name} variant='h4' className='flex flex-col gap-3' breadcrumbs={[...BREADCRUMBS['the-sims-four-pc-element-detail'], { label: data.name }]}>
      ...
      {/* Metals */}
      {data.metals.length > 0 && <DetailGrid name="Metals" data={data.metals.map(x => ({
        id: x.id,
        image: x.image,
        title: x.name,
        link: `/the-sims/four-pc-metal/${x.id}`
      }))} />}

      {/* Crystals */}
      {data.crystals.length > 0 && <DetailGrid name="Crystals" data={data.crystals.map(x => ({
        id: x.id,
        image: x.image,
        title: x.name,
