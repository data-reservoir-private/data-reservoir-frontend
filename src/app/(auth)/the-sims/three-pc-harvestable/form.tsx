...
export type ThreePCHarvestableFormSchema = z.infer<typeof schema>;

export default function ThreePCHarvestableForm({ param, totalData = 0 }: { param: ThreePCHarvestableFormSchema, totalData: number }) {
  const defaultValues = formOptions({
    ...
    } as ThreePCHarvestableFormSchema,
    validators: {
      onChange: schema
    },
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value, meta }) => {
      ...
      router.push(`/the-sims/three-pc-harvestable?${makeSearchParam(value)}`);
    }
  });
