...
type FourPCHarvestableFormSchema = z.infer<typeof schema>;
const defaultValue: FourPCHarvestableFormSchema = { name: "", form: [], rarity: undefined };

export default function FourPCHarvestableClient({ data }: { data: ITheSimsResponse['four-pc-harvestable'][] }) {
  const [state, setState] = useState<FourPCHarvestableFormSchema>(defaultValue);
  ...
  const realData = data.filter(x => (
    ...
    ((state.form ?? []).length === 0 || (state.form ?? []).includes(x.form)) &&
    ...
    <form.AppForm>
      <form.SimpleContainer className="flex flex-col grow gap-2">
        <Box className="gap-2 flex grow">
          ...
          <form.AppField name='form' children={(field) => (
            <field.SimpleMultiselectString label='Form' choices={CATEGORIES['the-sims-four-pc-harvestable']}/>
          )} />
          <form.AppForm>
      ...
      <SimpleGrid data={realData} link='/the-sims/four-pc-harvestable'/>
    </form.AppForm>
  )
}
