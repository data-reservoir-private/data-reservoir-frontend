...
export default async function ThreePCRawGem(props: ThreePCRawGemProps) {
  const { id } = await props.params;
  const { data } = await grabDetail(id);
  if (!data) return notFound();

  return (
    <Section name={data.name} variant='h4' className='flex flex-col gap-3' breadcrumbs={[...BREADCRUMBS['the-sims-three-pc-raw-gem-detail'], { label: data.name }]}>
      {/* Image */}
      ...
      {/* Gems */}
      {data.gemCuts.length > 0 && <DetailGrid name="Gems" data={data.gemCuts.map(x => ({
        id: x.id,
        image: x.image,
        title: x.name,
