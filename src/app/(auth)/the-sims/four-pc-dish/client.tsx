...
type FourPCDishFormSchema = z.infer<typeof schema>;
const defaultValue: FourPCDishFormSchema = { name: "", category: [] };

export default function FourPCDishClient({ data }: { data: ITheSimsResponse['four-pc-dish'][] }) {
  const [state, setState] = useState<FourPCDishFormSchema>(defaultValue);
    ...
    <form.AppForm>
      <form.SimpleContainer className="flex max-md:flex-col grow gap-2">
        ...
        <form.AppField name='category'>
          {(field) => <field.SimpleMultiselectString label='Category' choices={CATEGORIES['the-sims-four-pc-dish-type']} />}
        </form.AppField>
        <form.AppForm>
          <form.SimpleSubmitButton label='Search' />
        </form.AppForm>
      </form.SimpleContainer>
      <SimpleGrid data={realData} link='/the-sims/four-pc-dish' />
    </form.AppForm>
  );
}
