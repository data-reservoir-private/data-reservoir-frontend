...
export type ShippableFormSchema = z.infer<typeof schema>;

export default function ShippableForm({ param, totalData = 0 }: { param: ShippableFormSchema, totalData: number }) {
  const defaultValues = formOptions({
    defaultValues: {
      name: param.name ?? "",
      currentPage: param.currentPage ?? 1,
      pageSize: param.pageSize ?? 50
    } as ShippableFormSchema,
    validators: {
      onChange: schema
    },
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value, meta }) => {
      ...
      router.push(`/quartz/shippable?${makeSearchParam(value)}`);
    }
  });
