...
export type UtensilFormSchema = z.infer<typeof schema>;

export default function UtensilForm({ param, totalData = 0 }: { param: UtensilFormSchema, totalData: number }) {
  const defaultValues = formOptions({
    defaultValues: {
      name: param.name ?? "",
      currentPage: param.currentPage ?? 1,
      pageSize: param.pageSize ?? 50
    } as UtensilFormSchema,
    validators: {
      onChange: schema
    },
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value, meta }) => {
      ...
      router.push(`/quartz/utensil?${makeSearchParam(value)}`);
    }
  });
