...
export const Route = createFileRoute('/(header)/_header/export/$category/$data/$type')({
  server: {
    handlers: {
      GET: async ({ params: { category: categoryStr, data: dataIDStr, type: exportType } }) => {
        ...
        if (!d || !d.export || !d.export.exportType.includes(exportType as ExportType)) throw notFound();

        const { data: resTemp } = await grabData<object[]>(d.export!.route, {
          PageSize: 0
        });
        ...
        if (exportType === 'json') return new Response(JSON.stringify(res));
        if (exportType === 'ndjson') return new Response(res.map(x => JSON.stringify(x)).join('\n'), {
          headers: {
            'Content-Type': 'text/plain'
          }
        });
        else if (exportType === 'xml') return new Response(XML.js2xml({
        ...
        else if (exportType === 'yaml') return new Response(
        ...
        else if (exportType === 'csv' || exportType === 'tsv') return new Response(
          json2csv(res, {
            delimiter: {
          ...
          return new Response(await toHtmlTable(res), {
            headers: {
          ...
          return new Response(await toExcel(res), {
            headers: {
          ...
          return new Response(toDatabase(res, exportType), {
            headers: {
              'Content-Type': 'text/plain',
            }
          });
          ...
          return new Response((await toParquet(
