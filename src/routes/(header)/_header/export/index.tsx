...
const apiCall = createServerFn()
  .handler(async () => await grabData<IDashboardResponse[]>(`${API_ROUTE.DASHBOARD}`));

export const Route = createFileRoute('/(header)/_header/export/')({
                    ...
                    <Box className="flex justify-end gap-2 flex-wrap">
                      {
                        l.export!.exportType.map(et => (
                          <Link to='/export/$category/$data/$type' params={{ category: categoryKey, data: l.id, type: et }} key={et} target='_blank' >
